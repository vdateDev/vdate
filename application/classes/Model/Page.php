...
class Model_Page extends ORM {
    ...
    public function edit_page($data, $languages) {
        ...
        $values = Arr::extract($data, array('status', 'url', 'updated_at','show_footer'));
        $this->values($values);
        $this->update();
    ...
    public static function get_backend_pages() {
        ...
        $pages = self::factory('Page')
                    ->join('pages_languages')
                    ->on('page.id', '=', 'pages_languages.page_id')
                    ->select(
                        array('pages_languages.name', 'name')
                    )
                    ->where('pages_languages.language', '=', 'en')
                    ->order_by('page.sort', 'ASC')
                    ->find_all();
