$(document).ready(function() {
    ...
    fileInput.damnUploader({
        // куда отправлять
        url: '/backend/ajax/upload_prices',
    ...
    function updateInfo() {
        countInfo.text( (imgCount == 0) ? 'Файлов не выбрано' : ('Файлов выбрано: '+imgCount));
    ...
    function addFileToQueue(file) {
        ...
        if(!file.fake) {
            ...
            log('Файл добавлена: `'+file.name + '` (' +Math.round(file.size / 1024) + ' Кб)');
            imgSize += file.size;
        } else {
